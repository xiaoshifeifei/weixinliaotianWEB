<template>
  <div class="lum-dialog-mask">
    <el-container class="lum-dialog-box" style="height: 95%;max-height: 600px;" :class="{ 'full-screen': fullscreen }">
      <el-header height="60px" class="header">
        ...
        <p class="tools">
          <i class="iconfont" style="transform: scale(0.85)" :class="fullscreen ? 'icon-tuichuquanping' : 'icon-quanping'"
      ...
      <el-header height="38px" class="sub-header">
        ...
        <div class="search-box no-select">
          ...
          <input v-model="search.keyword" type="text" maxlength="30" placeholder="关键字搜索"
        ...
        <el-aside width="200px" class="broadside" v-show="broadside">
          <el-container class="full-height">
            <el-header height="40px" class="aside-header">
              <div class="item" :class="{ selected: contacts.show == 'friends' }" @click="contacts.show = 'friends'">
                我的好友({{ contacts.friends.length }})
              </div>
              <div class="item-shuxian">|</div>
              <div class="item" :class="{ selected: contacts.show == 'groups' }" @click="contacts.show = 'groups'">
                我的群组({{ contacts.groups.length }})
              </div>
            </el-header>
            <el-main class="no-padding">
              <el-scrollbar class="full-height" tag="section" :native="false">
                <div v-for="item in contacts[contacts.show]" class="contacts-item pointer" :class="{
                  selected:
                    findSource == item.type && findchatId == item.id,
                }" :key="item.id" @click="triggerMenuItem(item)">
                  <div class="avatar">
                    <el-avatar :size="20" :src="item.avatar">
                      <img src="~@/assets/image/detault-avatar.jpg" />
                    </el-avatar>
                  </div>
                  <div class="content" v-text="item.nickname"></div>
                </div>
              </el-scrollbar>
            </el-main>
          </el-container>
        ...
        <el-main v-show="showBox == 0" class="no-padding">
          <el-container class="full-height">
            <el-header height="40px" class="type-items">
              <span :class="{ active: records.msgType == 0 }" @click="triggerLoadType(0)">全部
              </span>
              <!--<span-->
              ...
              <!--&gt;代码块-->
              <!--</span>-->
              <span :class="{ active: records.msgType == 2 }" @click="triggerLoadType(2)">文件
              </span>
              <span :class="{ active: records.msgType == 4 }" @click="triggerLoadType(4)">会话记录
              </span>
            </el-header>
            ...
            <el-main v-else class="history-record">
              <el-scrollbar class="full-height" tag="section" :native="false">
                <div v-for="record in records.items" :key="record.id" class="message-group">
                  <div class="left-box">
                    <el-avatar shape="square" fit="contain" :size="30" :src="record.avatar" />
                  </div>
                  ...
                  <div class="right-box">
                    <div class="msg-header">
                      ...
                      <span class="time">{{ sendTime(record.createTime) }}</span>
                    </div>

                    <!-- 文本消息 -->
                    <text-message v-if="record.msg_type == 1" :content="record.content" />

                    <!-- 文件 - 图片消息 -->
                    <image-message v-else-if="
                      record.msg_type == 2 && record.file.file_type == 1
                    " :src="record.file.file_url" />

                    <!-- 文件 - 音频消息 -->
                    <audio-message v-else-if="
                      record.msg_type == 2 && record.file.file_type == 2
                    " :src="record.file.file_url" />

                    <!-- 文件 - 视频消息 -->
                    <video-message v-else-if="
                      record.msg_type == 2 && record.file.file_type == 3
                    " />

                    <!-- 文件 - 其它格式文件 -->
                    <file-message v-else-if="
                      record.msg_type == 2 && record.file.file_type == 4
                    " :file="record.file" :record_id="record.id" />

                    <!-- 会话记录消息 -->
                    <forward-message v-else-if="record.msg_type == 4" :forward="record.forward" :record_id="record.id" />

                    <!-- 代码块消息 -->
                    <code-message v-else-if="record.msg_type == 5" :code="record.code_block.code"
                      :lang="record.code_block.code_lang" />

                    <div v-else class="other-message">{{ record.content }} | {{ sendTime(record.createTime) }}</div>
                  </div>
                </div>
...
<script>
import { formateSize as renderSize, download, imgZoom, formateTime } from '@/utils/functions'
...
export default {
  ...
  mounted() {
    ...
    this.findSource = this.chatType
    this.findchatId = this.chatId
  ...
  computed: {
    endTime() {
      ...
      return (items.length == 0 ? 0 : items[items.length - 1].timeSend)
    }
  },
  methods: {
    download,
    ...
    // 获取好友列表
    loadFriends() {
      this.contacts.friends = this.$store.state.contact.friends
    },

    // 获取群聊列表
    loadGroups() {
      this.contacts.groups = this.$store.state.contact.rooms
    },
    ...
    loadChatRecord() {
      ...
      let data = {
        ...
        pageIndex: this.records.pageIndex,
        chatId: this.findchatId ? this.findchatId : this.chatId,
      ...
      chatApi.pageMsg(data)
        .then(res => {
          ...
          this.records.pageIndex++;

          this.records.items = records
          this.records.loadStatus =
            res.data.length < res.data.limit ? 2 : 0
