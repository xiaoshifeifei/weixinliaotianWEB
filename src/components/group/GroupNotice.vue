      ...
      <el-main class="main no-padding">
        ...
        <template v-else>
          <el-scrollbar class="full-height" :native="false" tag="section">
            ...
            >
              <div class="title">
                <span class="left-title max-length">{{ item.text }}</span>
                <span
              ...
              <p class="datetime">
                <el-avatar :size="15" :src="$root.getAvatar(item.userId)">
                  <img src="~@/assets/image/detault-avatar.jpg" />
                </el-avatar>
                <span
                  class="text nickname"
                  @click="$refs.userBusinessCard.open(item.userId)"
                >
                  {{ item.nickname }}
                </span>
                <span class="text">发表于 {{ $root.beautifyTime(item.time) }}</span>
              </p>
  ...
  methods: {
    ...
    loadNotices() {
      ...
      roomApi.listNotice({
        ...
        .then(res => {
          if (res.resultCode == 1) {
            this.items = res.data.pageData.map(item => {
              item.isShow = false
              return item
            })
