...
import {fileUtils} from "@/utils/my/fileUtils";
...
export default {
  ...
  methods: {
    ...
    uploadService() {
      ...
      let file = fileUtils.dataURLtoFile(this.cusPreviewsImg, this.fileName);
      fileApi.uploadAvatar(file, this.chatId)
      .then(res => {
        if (res.resultCode == 1) {
          imUtils.refreshVersion(this.chatId);
          busService.emit(eventConst.refreshAvatar, this.chatId)
          this.$emit('close', 1)
        } else {
