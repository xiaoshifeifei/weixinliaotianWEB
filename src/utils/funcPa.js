 ...
 * @param {String} params
 */
export function formateTime(datetime, params) {
	if (datetime == null || datetime == 0) {
		if (typeof params == 'string') {
			params = params.replace(/-/g, '/');
		} else if (/^[1-9]\d*$/.test(params)) {
			outTime = new Date(parseInt(params) * 1000);
		}

		let time = new Date();
		let outTime = new Date(params);
		...
		if (Number(datetime) > 0 || Number(datetime) < 0) {
			if (typeof datetime == 'string') {
				datetime = datetime.replace(/-/g, '/');
			} else if (/^[1-9]\d*$/.test(datetime)) {
 ...
 * @param {Object} datetime
 */
export function szTime(datetime) {
	if (datetime == null) return '';
	let date = new Date(datetime * 1000); // 参数需要毫秒数，所以这里将秒数乘于 1000
