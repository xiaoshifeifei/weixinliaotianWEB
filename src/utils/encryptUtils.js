...
export const EncryptUtils = {
	...
	*/
	buildLoginPassword: function(pwd) {
		...
		let encryptAES = EncryptUtils.encryptAES(
			CryptoJS.enc.Base64.parse(md5Str),
			md5pwd
		);
	...
	*/
	DH: {
		...
		*/
		...
		ecdh: null /*DH 对象*/,
		getECDH: function() {
			if (!this.ecdh) this.ecdh = new ec('secp256k1');
			return this.ecdh;
		},
		/*
		创建DH 密钥对
	...
	*/
	...
	rsaDecrypt: function(data, privateKeyData) {
		...
		return forge.util.encode64(
			this.fromPrivateKeyData(privateKeyData).decrypt(data)
		);
	},
