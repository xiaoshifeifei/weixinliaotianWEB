<template>
	<div class="body-bag" :class="themeBagImg">
		<el-container class="main-layout" :class="{ 'full-mode': themeMode }">
			<el-aside width="70px" class="side-edge" style="overflow: hidden">
				<el-container class="full-height">
					<el-header height="100px" class="logo-header">
						<!-- v-popover:usercard-->
						<div class="userlogo" @contextmenu="showMenu">
			...
			<el-main class="no-padding" style="background: white">
				<div class="rightBox" v-if="$store.state.user.isPc">
					<img
						@click="closeWindow"
...
export default {
	...
	methods: {
		...
		toContact() {
			this.$router.push('/index/contacts');
			this.$store.commit('set_apply_num', 0);
		},
		play() {
			document
				.querySelector('#audio')
		...
		logout() {
			var that = this;
			this.$confirm(
				`此操作将 <span style="color:red;font-size:16px;">退出登录</span>, 是否继续?`,
				'提示',
				{
