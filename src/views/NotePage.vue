  ...
  methods: {
    ...
    loadNoteDetail(id, isEdit) {
      ...
      ServeGetArticleDetail({
        ...
        .then(res => {
          if (res.resultCode == 1 && this.noteDetail.loadId == id) {
            this.loadStatus = 1
            ...
            this.noteDetail.createTime = res.data.createTime
          ...
          } else if (res.resultCode != 1 && this.noteDetail.loadId == id) {
            this.loadStatus = 2
          }
        })
        .catch(() => {
    ...
    loadNoteList() {
      ...
      ServeGetArticleList(data)
        .then(({ code, data }) => {
          ...
          this.notes = data.pageData.map(item => {
            return {
              id: item.id,
              title: item.title,
              datetime: parseTime(item.updateTime, '{y}/{m}/{d}'),
    ...
    loadNoteClass(class_id = null) {
      ServeGetArticleClass().then(({ code, data }) => {
        ...
        this.menus[2].submenus = data.pageData.map(item => {
          return {
            id: item.id,
